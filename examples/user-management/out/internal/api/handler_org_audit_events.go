...
// OrgAuditEventsHandler handles HTTP requests for AuditEvent entities.
type OrgAuditEventsHandler struct {
	store Storage
}

// NewOrgAuditEventsHandler creates a new OrgAuditEventsHandler.
func NewOrgAuditEventsHandler(store Storage) *OrgAuditEventsHandler {
	return &OrgAuditEventsHandler{store: store}
}

func (h *OrgAuditEventsHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(presentEntity(auditevent, "AuditEvent", auditevent.ID, "/api/user-mgmt/v1/org-events"+"/"+auditevent.ID))
}

func (h *OrgAuditEventsHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	opts := ListOptions{Page: page, Size: pageSize, OrderBy: orderBy, Fields: fields, Search: searchExpr, ImplicitFilters: map[string]string{"source_type": "Organization"}}
	...
	w.Header().Set("Content-Type", "application/json")
	hrefBase := "/api/user-mgmt/v1/org-events"
