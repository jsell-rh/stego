...
func (s *Store) listQuery(ctx context.Context, entity, scopeField, scopeValue string, opts stegostorage.ListOptions, afterID string, cursor bool) (stegostorage.ListResult, error) {
	...
	case "OrgSetting":
		...
		return stegostorage.ListResult{Items: result, Total: total}, nil
	case "AuditEvent":
		validCols := map[string]bool{"id": true, "created_time": true, "updated_time": true, "source_type": true, "source_id": true, "action": true, "detail": true}
		...
		query, err := s.applyRelated(ctx, query, "AuditEvent", opts.Related)
		if err != nil {
			return stegostorage.ListResult{}, err
		}
		query, err = s.applyRowFilter(ctx, query, "AuditEvent", opts.Filter)
		if err != nil {
			return stegostorage.ListResult{}, err
		}
		if scopeField != "" && scopeValue != "" {
			if !validCols[scopeField] {
				return stegostorage.ListResult{}, fmt.Errorf("invalid scope field %q for entity AuditEvent", scopeField)
			}
			query = query.Where(scopeField+" = ?", scopeValue)
		}
		for _, field := range filterKeys(opts.ImplicitFilters) {
			value := opts.ImplicitFilters[field]
			if !validCols[field] {
				return stegostorage.ListResult{}, fmt.Errorf("invalid implicit filter field %q for entity AuditEvent", field)
			}
			query = query.Where(field+" = ?", value)
		}
		if opts.Search != "" {
		...
		if !cursor {
			...
			if opts.CountOnly {
				return stegostorage.ListResult{Items: []AuditEvent{}, Total: total}, nil
			}
		}
		if len(opts.Fields) > 0 {
			// Always include id; add requested fields that exist.
...
func filterColumns(entity string) map[string]bool {
	...
	case "AuditEvent":
		return map[string]bool{"id": true, "created_time": true, "updated_time": true, "source_type": true, "source_id": true, "action": true, "detail": true}
	}
	return nil
}
...
func textColumns(entity string) map[string]bool {
	...
	case "AuditEvent":
		return map[string]bool{"source_type": true, "source_id": true, "action": true}
	}
	return nil
}
...
func (s *Store) relatedExpression(ctx context.Context, target string, filter stegostorage.RelatedFilter) (clause.Expression, error) {
	...
	case "AuditEvent":
		related = s.db.WithContext(ctx).Model(&AuditEvent{}).Select(filter.ForeignField)
	default:
