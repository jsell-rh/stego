...
import (
	...
	cursorcontract "github.com/example/service/out/contracts/storage"
)
...
func (s *Store) ReadCursor(ctx context.Context, entity, scopeField, scopeValue string, opts cursorcontract.CursorOptions) (cursorcontract.CursorResult, error) {
	...
	switch entity {
	case "Organization":
		columns = map[string]bool{"id": true, "created_time": true, "updated_time": true, "name": true, "description": true}
	case "User":
		columns = map[string]bool{"id": true, "created_time": true, "updated_time": true, "email": true, "display_name": true, "role": true, "org_id": true, "metadata": true}
	case "OrgSetting":
		columns = map[string]bool{"id": true, "created_time": true, "updated_time": true, "org_id": true, "key": true, "value": true, "generation": true}
	case "AuditEvent":
		columns = map[string]bool{"id": true, "created_time": true, "updated_time": true, "source_type": true, "source_id": true, "action": true, "detail": true}
	default:
		return cursorcontract.CursorResult{}, cursorcontract.ErrCursor
	}
	...
	switch entity {
	case "Organization":
		rows, ok := result.Items.([]Organization)
	...
	case "User":
		...
		return cursorcontract.CursorResult{Items: rows, NextID: next, More: more}, nil
	case "OrgSetting":
		rows, ok := result.Items.([]OrgSetting)
		if !ok || len(rows) > opts.Limit+1 {
			return cursorcontract.CursorResult{}, cursorcontract.ErrCursorResult
		}
		ids := map[string]bool{}
		...
		return cursorcontract.CursorResult{Items: rows, NextID: next, More: more}, nil
	case "AuditEvent":
		rows, ok := result.Items.([]AuditEvent)
		if !ok || len(rows) > opts.Limit+1 {
			return cursorcontract.CursorResult{}, cursorcontract.ErrCursorResult
		}
		ids := map[string]bool{}
