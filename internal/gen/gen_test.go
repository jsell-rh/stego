...
func TestValidateHeaders_AllValid(t *testing.T) {
	files := []File{
		{Path: "a.go", Content: []byte(Header + "\n\npackage a")},
		{Path: "b.go", Content: []byte(Header + "\n\npackage b")},
	}
	if err := ValidateHeaders(files); err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
}
...
func TestValidateHeaders_MissingHeader(t *testing.T) {
	files := []File{
		...
		{Path: "c.go", Content: []byte("// wrong header\n\npackage c")},
	}
	err := ValidateHeaders(files)
	if err == nil {
		t.Fatal("expected error for files missing header")
	}
	...
	if len(hdrErr.Violations) != 2 {
		t.Fatalf("expected 2 violations, got %d: %v", len(hdrErr.Violations), hdrErr.Violations)
	}
	if hdrErr.Violations[0] != "b.go" || hdrErr.Violations[1] != "c.go" {
		t.Errorf("unexpected violations: %v", hdrErr.Violations)
	}
}

func TestValidateHeaders_EmptyFileList(t *testing.T) {
	if err := ValidateHeaders(nil); err != nil {
		t.Fatalf("expected no error for empty file list, got: %v", err)
	}
}

func TestHeaderErrorMessage(t *testing.T) {
	e := &HeaderError{Violations: []string{"a.go", "b.go"}}
	msg := e.Error()
	want := `files missing generated header "// Code generated by stego. DO NOT EDIT.": a.go, b.go`
	if msg != want {
		t.Errorf("got: %s\nwant: %s", msg, want)
	}
}
