...
func TestFileBytes_IncludesHeader(t *testing.T) {
	f := File{
...
func TestFileBytes_EmptyContent(t *testing.T) {
	f := File{Path: "internal/api/empty.go", Content: []byte{}}
	got := string(f.Bytes())
	want := Header + "\n\n"
	if got != want {
		t.Errorf("Bytes() with empty content =\n%q\nwant:\n%q", got, want)
	}
}
