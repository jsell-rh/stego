...
func TestLiveRegistryLoadsAllArchetypeComponents(t *testing.T) {
	...
	if restAPI.Version != "3.0.2" {
		t.Errorf("rest-api Version = %q, want %q", restAPI.Version, "3.0.2")
	}
	...
	if pg.Version != "3.14.0" {
		t.Errorf("postgres-adapter Version = %q, want %q", pg.Version, "3.14.0")
	}
