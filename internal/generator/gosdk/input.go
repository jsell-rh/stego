...
func sourceNames(config map[string]any) (string, []string, error) {
	for key := range config {
		if key != "document" && key != "references" {
			return "", nil, fmt.Errorf("unsupported go-sdk configuration field %q", key)
		}
	}
	root, ok := config["document"].(string)
	if !ok || gen.ValidatePath(root) != nil {
		return "", nil, fmt.Errorf("go-sdk requires a project-relative document")
	}
	names := []string{root}
	...
	if value, present := config["references"]; present {
		refs, ok := value.([]any)
		if !ok || len(refs) > 63 {
			return "", nil, fmt.Errorf("go-sdk references must contain at most 63 paths")
		}
		for _, value := range refs {
			name, ok := value.(string)
			if !ok || gen.ValidatePath(name) != nil || seen[name] {
				return "", nil, fmt.Errorf("invalid or duplicate go-sdk reference path")
			}
			seen[name] = true
...
func loadDocument(ctx gen.Context) (*openapi3.T, error) {
	...
	if !strings.HasPrefix(doc.OpenAPI, "3.0.") {
		return nil, fmt.Errorf("go-sdk supports OpenAPI 3.0")
	}
	...
	if doc.Paths.Len() == 0 || doc.Paths.Len() > 256 {
		return nil, fmt.Errorf("go-sdk requires 1 through 256 paths")
	}
	operations := 0
