...
//go:embed process_health.go.tmpl
var processHealthSource []byte
...
func processFiles(ctx gen.Context) ([]gen.File, error) {
	...
	health, err := format.Source(processHealthSource)
	if err != nil {
		return nil, err
	}
	result := []gen.File{{Path: path.Join(ctx.OutputNamespace, "process/runtime.go"), Content: code}, {Path: path.Join(ctx.OutputNamespace, "process/health.go"), Content: health}}
	for _, p := range entries {
