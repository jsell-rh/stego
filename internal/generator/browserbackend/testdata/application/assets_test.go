...
func TestCapturedApplicationAssets(t *testing.T) {
	...
	for _, backend := range []*Backend{f.backend, second} {
		for _, path := range []string{"/", "/records/record-1", "/index.html", "/assets/main.js"} {
	...
	require(t, send(second, "GET", "/assets/main.js", "", []*http.Cookie{c}, nil).Code == 401, "captured asset survived logout on another backend")
}
