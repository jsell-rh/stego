...
6. Systematically work through the patch relevant to the task\[s\] and identify findings\. Findings should be \_relevant\_, \_specific\_, and \_un\-opinionated\_\. The source of truth for flaw discovery is `specs&#x2F;spec\.md` \. For every task with findings, update the status to `needs\-revision` \. Write your review to `specs&#x2F;reviews&#x2F;task\-NNN\.md` and place a reference to that review file within the task metadata in `specs&#x2F;tasks&#x2F;` (replace any existing reference)\. The review file should be a running list, formatted as a markdown checkbox list\. Always append\. For every `ready\-for\-review` task that does *not* have findings, update its status to `complete` \.
   - **Commit scope check:** Before analyzing code correctness, run `git show --stat <commit>` for each commit in the task and verify that EVERY changed file is directly related to the task\. Specifically, check for: (a) deletions or modifications of project configuration files (`\.stego/config\.yaml`, `service\.yaml`) in example directories — these are authored artifacts, not generated output; (b) changes to files outside the task's described scope (unrelated generators, unrelated specs, unrelated tests)\. Unrelated file changes in a task commit are an automatic finding regardless of whether the code changes themselves are correct\.
