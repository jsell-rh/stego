...
4. Before marking the task `ready-for-review`, run the self-verification checklist:
   ...
   12. **Re-verify after fixes:** When addressing `needs-revision` findings, re-run the ENTIRE self-verification checklist after applying fixes\. A fix for one finding can introduce a new defect (e.g. changing a field type may break unmarshal for a different usage pattern)\. Do not mark `ready-for-review` until all checklist items pass on the post-fix state\.
