...
4. Before marking the task `ready-for-review`, run the self-verification checklist:
   ...
   6. **Acceptance criteria traceability:** Re-read every acceptance criterion and every behavioral requirement stated in the task description (e.g. "Return structured errors with file path and line context"). For each one, identify the exact line(s) of code that satisfy it. If you cannot point to code that implements a stated requirement, the task is not complete. Do not rely on your memory of what you wrote — grep for the relevant structs, fields, and method signatures\.
