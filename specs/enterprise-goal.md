...
| --- | --- |
| C3: generation and delivery | Controlled offline inputs are implemented and checked: `stego build download` produces a zips-only module cache, and `stego build --module-cache` builds offline under `go.sum` enforcement with `dependency_proxy: "off"` records ([delivery evidence](application-delivery-evidence.json)). Automatic release qualification is implemented: `scripts/qualify-compiler-release.py` verifies the compiler checks and signed artifact runs at the exact commit, requires every job conclusion, and (with `--verify-release`) authenticates the published release through the consumer path and cross-checks its asset digests and evidence notes; [the qualification of `94d285d7`](compiler-release-qualification-evidence.json) and workflow run `36229617576` are evidence. The supported installation target is recorded: Linux on amd64 is the qualified platform (pinned `go1.26.8.linux-amd64` SDK, amd64 runners and checks, verified release assets); see [compiler-installation.md](compiler-installation.md). The production CA profile is partly worked: CA capture now records every root's notAfter date and the expired digests automatically, with the Baltimore CyberTrust Root reported as the single expired root in run `36233627201` (see [application-images.md](application-images.md)); operator selection of the production root set and live adoption remain open. Assessment on 2026-09-26 at Hypershell main `47aa9ed`: cert-manager automation does not remove the operator root-set decision. The Gateway already requests its public certificate from a `cert-manager.io/v1` `Certificate` bound to the operator-set `PublicIssuer` `ClusterIssuer` (`ensurePublicTLS`, `internal/gatewayworkload/public_tls.go`) and verifies the issued secret against the operator-supplied `PublicCAFile` bundle (`publicTrust`). cert-manager automates issuance; trust pinning remains an explicit operator input, per the recorded decision that internal trust is a separate explicit input. The remaining C3 public-TLS item is therefore an operational configuration act (select the ClusterIssuer and supply the matching CA bundle), not a product or compiler gap. Retain interruption, conflicting-edit, and concurrent-apply tests. |
...
| C5: storage and events | Complete transaction, migration, durable delivery, recovery, backup, and restore coverage. Whole-database rollback detection is implemented (STEGO PR #3, merged `2f080620`): generated stores track `database_id` plus a `nextval` write epoch and reject restored or replaced databases with `ErrDatabaseRollback`, covered by four passing generated tests and full CI. Cross-process writer fencing is implemented (STEGO PR #4, merged `1c1a1c1d`): a `stego_schema.writer_lease` row is taken unconditionally by the newest first writer and latched as `ErrWriterFenced` after takeover; re-assertion inside every write transaction and direct versioned write reads the lease row instead of rewriting it, so concurrent serializable transactions do not conflict on that row, and an empty holder during re-assertion is this process's own takeover still in flight rather than a foreign writer; `WriterLeaseCheck` lets background workers stop claiming work; covered by takeover, direct-write, pre-take, absent-table, concurrent-writer, and in-flight-takeover generated tests plus compiler database-access and CI. The lease is optional per deployment through the `writer_lease` component setting (a `single` default when unset, `off` for applications that write from several live processes, such as a runtime plus independent tooling or a multi-replica deployment); `off` removes the lease table, fence code, and grants, while epoch and identity rollback detection stay active, covered by generated unfenced-mode tests. The backup/restore contract is implemented (STEGO PR #5, merged `434a5e98`): the compiler merges component backup declarations into `contracts/backup/backup.json`, and generated stores gain `ReadRestoreRecord` and `VerifyRestore` entry points that run the schema gate first and then require the recorded database identity and epoch to match exactly, so a partial restore mixing objects from different points in time is rejected; covered by compiler manifest tests and generated restore tests plus CI. Outbox blocked and dead-letter visibility with retention is implemented (STEGO PR #6, merged `f45e8cc9`): generated queues gain `Blocked`, `DeadLetters`, and `Purge` operator views over `stego_outbox.messages`, covered by generated Postgres tests and CI. The real-broker Kafka CI check is out of scope: Kafka is a generic STEGO registry component and Hypershell does not use it; the generated Kafka publisher tests run against kfake. The migration ledger is implemented (STEGO PR #7, merged `3e3c0936`): `schema_generation` stores gain `stego_schema.migrations(version, digest, applied_at)`, applied under the bootstrap transaction with fail-closed shape checks; `Migrate` and the new `ApplyMigration` enforce no re-application, no gaps, no foreign rows, no digest drift, and no rolled-back versions; the ledger joins the backup manifest so restores carry the migration history. The decision rejected golang-migrate because it records only version and dirty (no digest, no gap rejection) and cannot commit inside the bootstrap transaction. The ledger contract is documented in `specs/schema-migrations.md`. C5 is complete; the remaining Hypershell storage work moves to steps 6-10. The one-writer `replicas: 1` note is retired: the Hypershell acceptance tests at `014911f` assert the epoch and fence behavior (`epoch_rollback_test.go` rejects restored and replaced databases on write; `restore_record_test.go` verifies backup records and rejects restored databases; `TestWriterLeaseOffDeploymentRunsUnfenced` pins the lease-off deployment). Each Gateway writes only its own per-Gateway database through one process, and `Recreate` is kept as the workload-level single-writer control. |
| C6: runtime | Health, readiness, timeout, shutdown, and resource-limit coverage is complete with bounded evidence. The helper telemetry audit closed through three merged changes: standalone helper operation records (PR #8, `d40cfa5d`), keyed controller notice phases (PR #8), and outbox worker counter gauges reported through the common telemetry runtime (PR #9, `67768b1d`; see [outbox telemetry evidence](application-delivery-evidence.json)). The earlier recovered browser initialization failure is closed as not reproducible with the improved diagnostics: the incident (run `35176586562`, exit code 1 at `browser.NewBrowserBackend`) predates the staged startup events, its run artifacts and private log are no longer retained beyond a SHA-256 digest, so no retroactive cause can be proved; the staged `startup.step.completed` diagnostics built in response are deployed in both console modules, and the three latest live runs (`35921996477`, `36196581322`, `36208130589`) each recorded all six browser-backend instances with every stage `success` and no failed startup pair. C6 has no open item. |
...
The [cleanup timing review](cleanup-latency-review-20260921.md) records five
complete 100-account samples, the diagnostic observation that bounds the
delay inside Kubernetes, and the adopted parallel sandbox deletion that meets
