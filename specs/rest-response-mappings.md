...
Status: compiler candidate. Application adoption and the complete REST workflow
are not yet accepted. The requirements in `rest-response-mapping-plan.md` remain
in effect.
...
This candidate supports strings, booleans, matching integer and floating-point
...
cannot supply these states, so this candidate rejects such conversions. Explicit
